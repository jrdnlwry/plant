...
function weightedPick(rng, rules) { const total = rules.reduce((s, r) => s + r.weight, 0); let cursor = rng() * total; for (const rule of rules) {
    cursor -= rule.weight;
    if (cursor <= 0)
        return rule.value;
} return rules[rules.length - 1].value; }
function pixelKey(x, y) { return `${x},${y}`; }
function addPixel(pixels, x, y, fill) { const px = Math.round(x); const py = Math.round(y); if (px < 0 || px > 31 || py < 0 || py > 31)
    return; pixels.set(pixelKey(px, py), fill); }
function addBlock(pixels, x, y, w, h, fill) { for (let iy = 0; iy < h; iy++)
    for (let ix = 0; ix < w; ix++)
        addPixel(pixels, x + ix, y + iy, fill); }
function drawPixelLine(pixels, x1, y1, x2, y2, fill, thickness = 1) { const steps = Math.max(Math.abs(Math.round(x2 - x1)), Math.abs(Math.round(y2 - y1)), 1); for (let step = 0; step <= steps; step++) {
...
function generateLSystem(state, params) { const config = L_SYSTEM_PRESETS[state.plantType] || L_SYSTEM_PRESETS.fern; const rng = createRng(state.seed + params.stage * 1009 + Math.round(state.growthProgress) * 17); let sentence = config.axiom; for (let i = 0; i < params.iterations; i++) {
...
function turtlePixelsFromLSystem(state, params) { const { sentence, config } = generateLSystem(state, params); const rng = createRng(state.seed ^ 0xa53c9e7d ^ params.stage * 313); const pixels = new Map(); const stack = []; let turtle = { x: 16 + params.lean, y: 22, angle: config.startAngle + params.lean * 7, width: state.plantType === 'sapling' ? 2 : 1 }; const stepLength = config.step * params.stepScale; const angleJitter = 5 + params.stage; if (state.plantType === 'succulent') {
    ...
    stampFlower(pixels, 10 + i * 3 + params.lean, 14 - (i % 2), params.highlight, params.outline); return pixels; }
function checkRenderCompatibility(value) { if (!value || typeof value !== 'object')
    return { supported: false, reason: 'invalid-snapshot' }; const receivedVersion = value.rendererVersion; if (receivedVersion !== exports.SUPPORTED_RENDERER_VERSION)
    return { supported: false, reason: 'unsupported-renderer-version', receivedVersion, supportedVersion: exports.SUPPORTED_RENDERER_VERSION }; if (!(0, plant_core_1.isPlantStateSnapshot)(value))
    return { supported: false, reason: 'invalid-snapshot' }; return { supported: true }; }
function createPlantRenderModel(snapshot) { const compatibility = checkRenderCompatibility(snapshot); if (!compatibility.supported)
    throw new Error(`Cannot render plant: ${compatibility.reason}`); const params = deriveGrowthParameters(snapshot); const pixels = turtlePixelsFromLSystem(snapshot, params); const preset = plant_core_1.plantTypeDefinitions[snapshot.plantType]; return { rendererVersion: exports.SUPPORTED_RENDERER_VERSION, viewBox: '0 0 32 32', ariaLabel: `${preset.label} pixel L-system plant companion for ${snapshot.location || 'your location'}: ${snapshot.weatherSummary}`, opacity: params.opacity, pixels: Array.from(pixels.entries()).map(([key, fill]) => { const [x, y] = key.split(',').map(Number); return { x, y, fill }; }), pot: [{ x: 8, y: 21, width: 16, height: 2, fill: params.outline }, { x: 9, y: 23, width: 14, height: 1, fill: params.outline }, { x: 10, y: 24, width: 12, height: 5, fill: params.outline }, { x: 11, y: 29, width: 10, height: 1, fill: params.outline }, { x: 9, y: 21, width: 14, height: 1, fill: '#e0a14a' }, { x: 10, y: 22, width: 12, height: 1, fill: '#b86f35' }, { x: 11, y: 24, width: 10, height: 4, fill: '#b86f35' }, { x: 11, y: 24, width: 3, height: 4, fill: '#e0a14a' }, { x: 18, y: 25, width: 3, height: 3, fill: '#6b3f24' }, { x: 12, y: 29, width: 8, height: 1, fill: '#6b3f24' }] }; }
