...
import { biomeForState, canonicalPublicationSnapshot, PublicationValidationError, snapshotDigest, validateGardenPublicationRequest } from '../../../../../lib/garden/publication';
...
export async function POST(request: Request) {
  ...
  const { data: installation, error: installationError } = await admin.from('extension_installations')
    .select('installation_id,account_id,public_contributor_id,linked_at,revoked_at')
    .eq('installation_id', input.installationId).maybeSingle();
  if (installationError) return linkError('internal-error', 'Publication could not be completed.', 500, true);
  ...
  const [{ data: profile, error: profileError }, { data: contributor, error: contributorError }] = await Promise.all([
    admin.from('account_profiles').select('first_name,state_code').eq('account_id', installation.account_id).maybeSingle(),
    admin.from('public_contributors').select('public_id,visibility_status').eq('id', installation.public_contributor_id).maybeSingle(),
  ]);
  if (profileError || contributorError) return linkError('internal-error', 'Publication could not be completed.', 500, true);
  ...
  const canonicalSnapshot = canonicalPublicationSnapshot(input);
  const digest = snapshotDigest(input, canonicalSnapshot);
  ...
  if (data?.error) return linkError('internal-error', 'Publication could not be completed.', 500, true);
  if (error) {
  ...
  if (!data?.gardenPlantId || !data?.plotId || !data?.receiptId) return linkError('internal-error', 'Publication could not be completed.', 500, true);
